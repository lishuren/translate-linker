...
class TranslationService:
    ...
    def __init__(self):
        ...
        self.allow_user_model_selection = (
            os.getenv("ALLOW_USER_MODEL_SELECTION", "true").lower() == "true"
        )
        self.api_keys = APIKeySettings.from_env()
        self.upload_dir = Path(self.UPLOADS_DIR)
    ...
    def get_llm(self, provider: str = None, user_id: Optional[str] = None):
        ...
        if user_id and not self.user_settings_service.is_model_selection_allowed(
            user_id
        ):
        ...
        if not api_key:
            print(
                f"Warning: No API key for provider {provider}. Using default provider."
            )
            ...
            if not api_key:
                for test_provider in [
                    ...
                    if test_key:
                        ...
                        api_key = test_key
                        print(
                            f"Using {provider} as fallback provider with available API key"
                        )
                        break

        if not api_key:
            raise ValueError(
                f"No API keys configured for any provider. Please configure at least one LLM provider API key."
            )
        ...
        try:
            if provider == "openai" or provider == "chatgpt":
                ...
                return ChatOpenAI(
                    ...
                    temperature=float(os.getenv("LLM_TEMPERATURE", 0.1)),
                    openai_api_base=os.getenv(
                        "SILICONFLOW_API_BASE", "https://api.siliconflow.net/v1"
                    ),
                    openai_api_key=api_key,
                )
            else:
        ...
        try:
            ...
            try:
                ...
                translation_agent = await self._setup_translation_agent(
                    source_language, target_language, vector_store, llm
                )
            ...
            translated_chunks = await self._translate_with_agent(
                translation_agent, docs, source_language, target_language
            )
            ...
            if user_id:
                web_translation_service = (
                    self.user_settings_service.get_web_translation_service(user_id)
                )
            ...
            output_path = await self.file_service.save_translation(
                translation_id, final_translation, original_filename
            )
            ...
            confidence_score = self._calculate_confidence_score(
                source_language, target_language, len(docs)
            )
            ...
            translations_db[translation_id] = Translation(
                ...
                status=TranslationStatus.FAILED,
                createdAt=(
                    translations_db[translation_id].createdAt
    ...
    def _calculate_confidence_score(
        self, source_language: str, target_language: str, chunk_count: int
    ) -> float:
        ...
        if (source_language, target_language) in common_pairs or (
            target_language,
            source_language,
        ) in common_pairs:
            base_score += 0.05
    ...
    async def get_translation_file(self, translation_id: str) -> Optional[str]:
        ...
        if (
            translation_id in translations_db
            and translations_db[translation_id].status == TranslationStatus.COMPLETED
        ):
    ...
    async def _setup_translation_agent(
        self, source_language: str, target_language: str, vector_store: Any, llm: Any
    ) -> Any:
        ...
        translation_chain = LLMChain(
            llm=llm, prompt=translation_prompt, output_parser=StrOutputParser()
        )
        ...
        tools = [
            ...
            Tool(
                name="Translate Text",
                func=lambda text: translation_chain.invoke(
                    {
                        "source_language": source_language,
                        "target_language": target_language,
                        "text": text,
    ...
    async def delete_translation(self, translation_id: str, user_id: str) -> bool:
        """
        Delete a translation file and associated metadata.
        Returns True if successful, False if not found.
        """
        if not self.upload_dir.exists():
            return False

        # Construct the file path based on the translation ID and original filename
        potential_files = list(self.upload_dir.glob(f"{translation_id}_*"))
        if not potential_files:
            return False

        # Delete the file
        file_path = potential_files[
            0
        ]  # Take the first match (assuming unique ID ensures one file)
        try:
            os.remove(file_path)
            return True
        except Exception as e:
